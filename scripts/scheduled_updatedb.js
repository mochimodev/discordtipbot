...
module.exports.run = async (bot, con, guild_id, log_file) => {
	...
	mysqldump({
		...
		con.query(`SELECT userid from d_users`, (err, rows) => {
			...
			guild.members.forEach(member => {
				...
				if (ids_to_add.includes(member.user.id)) {
					let insert_query = `INSERT INTO d_users (username, userid, rank, available_mcm, received_mcm) VALUES ('${tip_receiver.username}', ${tip_receiver.id}, NULL, 0, 0)`
		...
		let log_msg = `\n${d_formatted}: !scheduled_updatedb was executed for ${month}. Dump filename: ${dump_filename}`
